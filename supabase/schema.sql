...
CREATE TABLE users (
  ...
  company_id    UUID,                        -- agentes: compañía asignada
  client_id     UUID,                        -- asegurados: cliente vinculado
  phone         TEXT,
  avatar_url    TEXT,
  pin_hash      TEXT,                        -- PIN 6 dígitos hasheado
...
-- ============================================================
-- CREDENCIALES BIOMÉTRICAS (WebAuthn)
-- ============================================================
...
CREATE TABLE risks (
  ...
  display_name   TEXT NOT NULL,          -- "Etios", "Casa Olivos"
  ...
  vin            TEXT,
  engine_no      TEXT,
  -- Datos inmueble (solo hogar/comercio)
...
CREATE TABLE documents (
  ...
  display_name    TEXT NOT NULL,          -- "Póliza Etios 6-26"
  period          TEXT NOT NULL,          -- "2026-06"
  year            INT NOT NULL,
  file_url        TEXT NOT NULL,          -- URL Supabase Storage
  onedrive_url    TEXT,                   -- URL OneDrive link anónimo
  onedrive_path   TEXT,                   -- ruta OneDrive
  file_size       BIGINT,
...
-- ============================================================
-- PUSH TOKENS (para alertas en celular)
-- ============================================================
...
-- USUARIOS: admin ve todo, cada uno ve solo su perfil
CREATE POLICY "users_admin_all" ON users
  FOR ALL USING (get_user_role() IN ('admin','coordinador'));

CREATE POLICY "users_self" ON users
  FOR SELECT USING (id = auth.uid());

-- COMPANIES: admin/coordinador/agente ven todas
CREATE POLICY "companies_staff" ON companies
  FOR ALL USING (get_user_role() IN ('admin','coordinador'));

CREATE POLICY "companies_agent_read" ON companies
  FOR SELECT USING (get_user_role() = 'agente');

CREATE POLICY "companies_client_read" ON companies
  FOR SELECT USING (get_user_role() = 'asegurado');

-- CLIENTES: admin ve todos, agente ve los de su compañía, asegurado se ve a sí mismo
CREATE POLICY "clients_admin_all" ON clients
  FOR ALL USING (get_user_role() IN ('admin','coordinador'));

CREATE POLICY "clients_agent_company" ON clients
  FOR SELECT USING (
    get_user_role() = 'agente' AND
    EXISTS (
      SELECT 1 FROM risks r
      WHERE r.client_id = clients.id
      AND r.company_id = get_user_company()
    )
  );

CREATE POLICY "clients_self" ON clients
  FOR SELECT USING (id = get_user_client());

-- RIESGOS: admin ve todos, agente ve los de su compañía, asegurado ve los suyos
CREATE POLICY "risks_admin_all" ON risks
  FOR ALL USING (get_user_role() IN ('admin','coordinador'));

CREATE POLICY "risks_agent_company" ON risks
  FOR ALL USING (
    get_user_role() = 'agente' AND company_id = get_user_company()
  );

CREATE POLICY "risks_client_self" ON risks
  FOR SELECT USING (
    get_user_role() = 'asegurado' AND client_id = get_user_client()
  );

-- DOCUMENTOS: misma lógica que riesgos
CREATE POLICY "docs_admin_all" ON documents
  FOR ALL USING (get_user_role() IN ('admin','coordinador'));

CREATE POLICY "docs_agent_company" ON documents
  FOR ALL USING (
    get_user_role() = 'agente' AND company_id = get_user_company()
  );

CREATE POLICY "docs_client_self" ON documents
  FOR SELECT USING (
    get_user_role() = 'asegurado' AND client_id = get_user_client()
  );
...
CREATE POLICY "payments_agent_company" ON payments
  FOR ALL USING (
    get_user_role() = 'agente' AND company_id = get_user_company()
  );

CREATE POLICY "payments_client_self" ON payments
  FOR SELECT USING (
    get_user_role() = 'asegurado' AND client_id = get_user_client()
  );

-- ALERTAS: cada usuario ve solo las suyas
...
-- PUSH TOKENS: solo el propio usuario
...
-- COTIZACIONES: admin ve todo, agente ve las de su compañía
CREATE POLICY "quotes_admin_all" ON quotes
  FOR ALL USING (get_user_role() IN ('admin','coordinador'));

CREATE POLICY "quotes_agent_company" ON quotes
  FOR ALL USING (
    get_user_role() = 'agente' AND company_id = get_user_company()
  );
...
-- Política storage: agentes pueden subir a su compañía, asegurados solo leer los suyos
CREATE POLICY "storage_documents_upload" ON storage.objects
  FOR INSERT WITH CHECK (
    bucket_id = 'documents' AND
    (SELECT role FROM users WHERE id = auth.uid()) IN ('admin','coordinador','agente')
  );
...
-- ============================================================
-- USUARIO ADMIN INICIAL (Luis)
-- Cambiá el email por el tuyo
-- ============================================================
-- NOTA: crear primero el usuario en Supabase Auth, luego ejecutar esto
-- INSERT INTO users (id, email, full_name, role, status)
-- VALUES ('UUID-DEL-AUTH', 'luis@argon.com.ar', 'Luis López Melo', 'admin', 'activo');
